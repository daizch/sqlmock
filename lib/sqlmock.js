...
function randomMock(row) {
    ...
    const StringSize = {
        ...
        char: 255,
        varchar: 255,
        mediumtext: 1e3, //16777215 too long
        longtext: 1e4 //4294967295 too long
    };
...
class SQLMock {
    ...
    generate(opts) {
        ...
        return new Promise((resolve, reject)=> {
            self.connection.query(`desc ${table}`, function (err, result) {
                ...
                if (opts.datasrc) {
                    let datasrc = (path.extname(opts.datasrc) === '.json') ? require(opts.datasrc) : fs.readFileSync(opts.datasrc).toString();

                    if (opts.dataFilter) {
                        rows = opts.dataFilter(datasrc);
                    } else {
                        rows = datasrc;
                    }
                } else {
     ...
     *
     * @param config
     * @param config.datasrc string filepath
